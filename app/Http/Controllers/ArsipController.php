...
class ArsipController extends Controller
{
    ...
    {
        $seksi = '"' . $request->seksi . '"';
        $suratKeluar = DB::select("SELECT no_surat,tanggal,tujuan,users.nama_seksi as nama,perihal,file_surat FROM surat_keluar INNER JOIN users ON users.id = surat_keluar.asal_surat WHERE users.nama_seksi =" . $seksi);
